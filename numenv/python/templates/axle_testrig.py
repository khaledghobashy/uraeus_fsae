...
class topology(object):
    ...
    def assemble(self, indicies_map, interface_map, rows_offset):
        ...
        self.jac_rows = np.array([0, 0, 0, 0, 1, 1, 1, 1], dtype=np.intc)
        self.jac_rows += self.rows_offset
        self.jac_cols = np.array([self.vbr_wheel_hub*2, self.vbr_wheel_hub*2+1, self.vbr_wheel_upright*2, self.vbr_wheel_upright*2+1, self.vbl_wheel_hub*2, self.vbl_wheel_hub*2+1, self.vbl_wheel_upright*2, self.vbl_wheel_upright*2+1], dtype=np.intc)
    ...
    def set_initial_states(self):
        np.concatenate([[]], out=self._q)

        np.concatenate([[]], out=self._qd)
    ...
    def _map_lagrange_multipliers(self):
        Lambda = self._lgr
        self.L_mcr_wheel_lock = Lambda[0:1]
        self.L_mcl_wheel_lock = Lambda[1:2]
    ...
    def eval_reactions_eq(self):
        ...
        Q_vbr_wheel_hub_mcr_wheel_lock = (-1) * multi_dot([np.bmat([[Z1x3.T],[multi_dot([((-1 * sin(config.UF_mcr_wheel_lock(t))) * B(self.P_vbr_wheel_hub,self.Mbar_vbr_wheel_hub_jcr_hub_bearing[:,0:1]).T + cos(config.UF_mcr_wheel_lock(t)) * B(self.P_vbr_wheel_hub,self.Mbar_vbr_wheel_hub_jcr_hub_bearing[:,1:2]).T),A(self.P_vbr_wheel_upright),self.Mbar_vbr_wheel_upright_jcr_hub_bearing[:,0:1]])]]),self.L_mcr_wheel_lock])
        self.F_vbr_wheel_hub_mcr_wheel_lock = Q_vbr_wheel_hub_mcr_wheel_lock[0:3]
        Te_vbr_wheel_hub_mcr_wheel_lock = Q_vbr_wheel_hub_mcr_wheel_lock[3:7]
        self.T_vbr_wheel_hub_mcr_wheel_lock = (0.5) * multi_dot([E(self.P_vbr_wheel_hub),Te_vbr_wheel_hub_mcr_wheel_lock])
        Q_vbl_wheel_hub_mcl_wheel_lock = (-1) * multi_dot([np.bmat([[Z1x3.T],[multi_dot([((-1 * sin(config.UF_mcl_wheel_lock(t))) * B(self.P_vbl_wheel_hub,self.Mbar_vbl_wheel_hub_jcl_hub_bearing[:,0:1]).T + cos(config.UF_mcl_wheel_lock(t)) * B(self.P_vbl_wheel_hub,self.Mbar_vbl_wheel_hub_jcl_hub_bearing[:,1:2]).T),A(self.P_vbl_wheel_upright),self.Mbar_vbl_wheel_upright_jcl_hub_bearing[:,0:1]])]]),self.L_mcl_wheel_lock])
        self.F_vbl_wheel_hub_mcl_wheel_lock = Q_vbl_wheel_hub_mcl_wheel_lock[0:3]
        Te_vbl_wheel_hub_mcl_wheel_lock = Q_vbl_wheel_hub_mcl_wheel_lock[3:7]
